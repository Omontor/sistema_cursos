    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.certificates.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('finished') ? 'is-invalid' : '' }}" type="text" name="finished" id="finished" value="{{ old('finished') }}" required>
                @if($errors->has('finished'))
                    <span class="text-danger">{{ $errors->first('finished') }}</span>
                @endif
