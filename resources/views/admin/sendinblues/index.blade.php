...
@can('sendinblue_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.sendinblues.create') }}">
                ...
                <tbody>
                    @foreach($sendinblues as $key => $sendinblue)
                        <tr data-entry-id="{{ $sendinblue->id }}">
                            ...
                            <td>
                                @can('sendinblue_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.sendinblues.show', $sendinblue->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('sendinblue_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.sendinblues.edit', $sendinblue->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('sendinblue_delete')
                                    <form action="{{ route('admin.sendinblues.destroy', $sendinblue->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sendinblue_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.sendinblues.massDestroy') }}",
