...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.test-results.update", [$testResult->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.testResult.fields.show') }}</label>
                <select class="form-control {{ $errors->has('show') ? 'is-invalid' : '' }}" name="show" id="show">
                    <option value disabled {{ old('show', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\TestResult::SHOW_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('show', $testResult->show) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('show'))
                    <span class="text-danger">{{ $errors->first('show') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.testResult.fields.show_helper') }}</span>
            </div>
            <div class="form-group">
