...
@can('index_about_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.index-abouts.create') }}">
                        ...
                        <tr data-entry-id="{{ $indexAbout->id }}">
                            ...
                            <td>
                                @can('index_about_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.index-abouts.show', $indexAbout->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('index_about_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.index-abouts.edit', $indexAbout->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('index_about_delete')
                                    <form action="{{ route('admin.index-abouts.destroy', $indexAbout->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('index_about_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.index-abouts.massDestroy') }}",
