...
@can('requirment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.requirments.create') }}">
                ...
                <tbody>
                    @foreach($requirments as $key => $requirment)
                        <tr data-entry-id="{{ $requirment->id }}">
                            ...
                            <td>
                                @can('requirment_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.requirments.show', $requirment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('requirment_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.requirments.edit', $requirment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('requirment_delete')
                                    <form action="{{ route('admin.requirments.destroy', $requirment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('requirment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.requirments.massDestroy') }}",
