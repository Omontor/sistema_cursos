...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.progress.update", [$progress->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $progress->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_finished') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="is_finished" id="is_finished" value="1" {{ $progress->is_finished || old('is_finished', 0) === 1 ? 'checked' : '' }} required>
