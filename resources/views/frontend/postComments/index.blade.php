        ...
        <div class="col-md-12">
            @can('post_comment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.post-comments.create') }}">
                        ...
                        <table class=" table table-bordered table-striped table-hover datatable datatable-PostComment">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        {{ trans('cruds.postComment.fields.status') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                                    ...
                                    <tr data-entry-id="{{ $postComment->id }}">
                                        ...
                                        <td>
                                            @can('post_comment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.post-comments.show', $postComment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('post_comment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.post-comments.edit', $postComment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('post_comment_delete')
                                                <form action="{{ route('frontend.post-comments.destroy', $postComment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('post_comment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.post-comments.massDestroy') }}",
