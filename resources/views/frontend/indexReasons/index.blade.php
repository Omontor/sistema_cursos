        ...
        <div class="col-md-12">
            @can('index_reason_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.index-reasons.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $indexReason->id }}">
                                        ...
                                        <td>
                                            @can('index_reason_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.index-reasons.show', $indexReason->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('index_reason_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.index-reasons.edit', $indexReason->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('index_reason_delete')
                                                <form action="{{ route('frontend.index-reasons.destroy', $indexReason->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('index_reason_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.index-reasons.massDestroy') }}",
