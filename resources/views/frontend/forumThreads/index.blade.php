        ...
        <div class="col-md-12">
            @can('forum_thread_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.forum-threads.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $forumThread->id }}">
                                        ...
                                        <td>
                                            @can('forum_thread_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.forum-threads.show', $forumThread->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('forum_thread_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.forum-threads.edit', $forumThread->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('forum_thread_delete')
                                                <form action="{{ route('frontend.forum-threads.destroy', $forumThread->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
